...
def prompt_date():
    ...
    while (testigo == True):
        testigo = False
        entrada = input("   -- Introduzca fecha (año[xxxx]-mes[xx]-día[xx]): ")
        try:
            #fecha = datetime.strptime(entrada, "%Y-%m-%d").date() # Tranformation text to date
            fecha = datetime.strptime(entrada, "%Y-%m-%d")    # Tranformation text to date
...
# Return the minimum, maximum, mean and standard deviation of a gived series
def min_max_mean_std_from_pd_series(pd_series):
    import pandas as pd
    out_describe_pd=pd.Series()
    
    try:
        if isinstance(pd_series, pd.Series):    #Check for the gived parameter
            describe_pd=pd_series.describe()
            out_describe_pd=[describe_pd['min'],describe_pd['max'],describe_pd['mean'],round(describe_pd['std'],2)]
            return out_describe_pd
    except:
        print("The parameter gived is not an array of pandas (pd.Series)")



# Sort by values and then by index
def sort_series(pd_series):
    import pandas as pd

    try:
        if isinstance(pd_series, pd.Series):
            pd_sorted_values_out=pd_series[pd_series>=5].sort_values()
            pd_sorted_index_out=pd_series[pd_series>=5].sort_index(ascending=False)

            return [pd_sorted_values_out,pd_sorted_index_out]
    except:
        print("The parameter gived is not an array of pandas (pd.Series)",ValueError)


# Receive a dataframe with 4 month of sales and expenses, then calculate the balance for each
# month and store into a Pandas Series to be returned
def balance(df):
    import pandas as pd
    balance_pd = pd.DataFrame({'Balance': [0, 0, 0, 0]}, index=df.index)

    for value in df.index:
        balance_pd.loc[value, 'Balance'] = df.loc[value, 'Ventas'] - df.loc[value, 'Gastos']

    return (balance_pd['Balance']).sort_values(ascending=True)



# Receive a dataframe of a csv file from a GitHub repository, and respecting its columns,
# it calculate and return a dataFrame with the minimum, maximum and mean of each column.
def dayly_key_indicators(df_fichero):
    import pandas as pd

    df_fichero=df_fichero.set_index('Nombre')   # Change the index to the column of enterprise

    columns_result=df_fichero.columns
    index_result=['min','max','mean']

    df_result = pd.DataFrame(np.zeros((3,5)),
                    index=index_result,
                    columns=columns_result)

    for column in df_fichero.columns:
        for operation in index_result:
            df_result.loc[operation,column]=df_fichero.describe().loc[operation][column]

    return df_result



def report_estacion_magnitud_rango_periodo(df,station,contaminant,start_date,final_date):
    ...
    if df_filtrado.empty:
        ...
        return df_filtrado  # it's an empty dataFrame
    else:
        df_fechas = df_filtrado[(df_filtrado['FECHA'] >= start_date) & (df_filtrado['FECHA'] <= final_date)]
        if df_fechas.empty:
            print(f"La solicitud de ESTACIÓN ({station}), MAGNITUD ({contaminant}) con fecha inicial ({start_date}) y fecha final ({final_date}) no existe en este dataFrame")
            return df_fechas # it's an empty dataFrame
        else:
            emissions = df_fechas['VALOR']
            return emissions