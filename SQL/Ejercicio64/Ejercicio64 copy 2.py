...
df=df.drop(columns=['PROVINCIA','MUNICIPIO','PUNTO_MUESTREO'])  # Eliminate columns that are not importants
df.to_csv('tabla_inicial_todos_los_años.csv')
print(df)
...
df = df[df['V01'].eq('V').all(axis=1)]


dataframe_columns = df.columns
#['ESTACION', 'MAGNITUD', 'ANO', 'MES','D01', 'V01', 'D02', 'V02', 'D03', 'V03', 'D04', 'V04', 'D05', 'V05', 'D06', 'V06', 'D07', 'V07', 'D08', 'V08', 'D09', 'V09', 'D10', 'V10', 'D11', 'V11', 'D12', 'V12', 'D13', 'V13', 'D14', 'V14', 'D15', 'V15', 'D16', 'V16', 'D17', 'V17', 'D18', 'V18', 'D19', 'V19', 'D20', 'V20', 'D21', 'V21', 'D22', 'V22', 'D23', 'V23', 'D24', 'V24', 'D25', 'V25', 'D26', 'V26', 'D27', 'V27', 'D28', 'V28', 'D29', 'V29', 'D30', 'V30', 'D31', 'V31']
lenght=len(df.loc[0])-5

df_final=pd.DataFrame
df_temporal=pd.DataFrame
indice=3    # For starting at column 5 ('V01)

for i in range(lenght):
    df_temporal = df[df[dataframe_columns[indice+i+1]].eq('V').all(axis=1)]
    if (indice+i>3):
        columns_to_erase=[4:]
    else:
        columns_to_erase=dataframe_columns[indice+i+1:]
    df_temporal = df.drop(columns=[ ])
